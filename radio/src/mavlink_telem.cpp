...
// -- LOGGING --
// hmmm this is not soo easy
#if 0

FIL g_mavlinkLogFile __DMA;


void mavlinkLogInit(void)
{
  memset(&g_mavlinkLogFile, 0, sizeof(g_mavlinkLogFile));
}


const char* mavlinkLogOpen(void)
{
  FRESULT result;
  ...
  if (sdGetFreeSectors() == 0) return STR_SDCARD_FULL;
  ...
  while (i > sizeof(LOGS_PATH) - 1) {
    if (!len && filename[i]) len = i+1;
    if (len) {
      if (filename[i]) filename[i] = zchar2char(filename[i]); else filename[i] = '_';
    }
    i--;
  }
  ...
  strcpy(tmp, "mlg"); //STR_LOGS_EXT);

  result = f_open(&g_mavlinkLogFile, filename, FA_OPEN_ALWAYS | FA_WRITE | FA_OPEN_APPEND);
...
{
  if (sdMounted()) {
    if (f_close(&g_mavlinkLogFile) != FR_OK) {
        g_mavlinkLogFile.obj.fs = 0;
    }
  }
}


// this does also all the checking, so don't go further if this returns false
bool mavlinkLogWriteDateTime(void)
{
  if (!sdMounted()) return false;

  if (!g_model.mavlinkLogging) {
      if (g_mavlinkLogFile.obj.fs) { //close it if open
          mavlinkLogClose();
      }
      return false;
  }

  if (!g_mavlinkLogFile.obj.fs) { //open it if needed
        const char* result = mavlinkLogOpen();
        if (result) return false;
  }

  struct gtm utm;
  gettime(&utm);
  f_printf(&g_mavlinkLogFile, "\r\n%4d-%02d-%02d,%02d:%02d:%02d.%02d0:", utm.tm_year+TM_YEAR_BASE, utm.tm_mon+1, utm.tm_mday, utm.tm_hour, utm.tm_min, utm.tm_sec, g_ms100);
